...
infiles = ['sap_stats.txt','pdc_stats.txt',
           'psf_stats.txt']#,'circ6_stats.txt',]#'circ6_stats.txt',
...
for ii,ifile in enumerate(infiles):
    print ifile
    try:
        tmag,s1,es1,s2,es2,sigma,ks,p,rms_diff,alpha,beta = sp.genfromtxt(ifile,unpack=1)
        ...
        ax6.plot(tmag,rms_diff/sp.sqrt(986)/sigma,'.',color=cuse[ii])
        ax7.plot(tmag,alpha,'.',color=cuse[ii])
        ax8.plot(tmag,beta,'.',color=cuse[ii])
