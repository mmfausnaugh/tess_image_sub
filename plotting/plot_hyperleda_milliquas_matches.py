...
def get_meta_data(ifile):
    ...
    obj = ifile.split('_')[-1]
    m = np.in1d(cat.source_id, int(obj))
...
def main():
    ...
    for ifile in args.infiles:
        #first load the file
        print(ifile)
        h_name, hyp_class, mil_class = np.genfromtxt(ifile,unpack=1,usecols=(0,2,3),
                                                     dtype=str,skip_header=1,
                                                     delimiter=',')
        h_name = np.array([h.strip() for h in h_name])
        hyp_class = np.array([h.strip() for h in hyp_class])
        mil_class = np.array([h.strip() for h in mil_class])

        imag = np.genfromtxt(ifile,unpack=1,usecols=(7),
                             skip_header=1, delimiter=',')


        #sector number
        sector_search = re.search('s(\d\d)',ifile)
        sector = sector_search.group(1)
        ...
        phot_file = os.path.join(dstem,'lc_hyperleda/phot.data')
        ...
        for ii,h in enumerate(h_name):
            outfile = os.path.dirname(ifile)
            outfile = os.path.join(outfile, 'lc_{}.png'.format(h))
            if os.path.isfile(outfile):
                continue
            fuse = os.path.join(dstem,
                                'lc_hyperleda/lc_{}_cleaned'.format(h) )
            ...
            plot_lc(x,y,z, ax1, imag[ii])

            plot_sky(x,y,z,bkg,bkg_mod, ax2, ax3, imag[ii])

            format_plot(ax1,h, hyp_class[ii], 
                        mil_class[ii], 
                        imag[ii], 
                        sector, cam, ccd )


            lookup_file = np.in1d(names, 'lc_' + h )
