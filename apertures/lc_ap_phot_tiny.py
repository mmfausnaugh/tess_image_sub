...
def do_phot(infile, photdata, plot=False):
    ...
    with fits.open(infile) as fhandle:
        ...
        for ii in range(np.shape(photdata)[1]):            
            ...
            with open(fname,'a') as fout_phot:
                ...
                #    if ii == 1 and plot==True:
                #        fout,bkg,col,row = p.compute_phot(ap_use, bkg_use, plot=True)
                #    else:
                ...
                #    fout_phot.write( fmt.format(fout,bkg,col,row) )
                    ...
                    #file
                flux, bkg =  p.lots_of_small_circle_phot(col_center - cmin, row_center-rmin)
                outstring = ''
                for ii in range(len(flux)):
                    outstring = outstring + '{:15.2f}{:15.2f}'.format(flux[ii],bkg[ii])
                fout_phot.write(outstring + '\n')
...
#            fout_phot.write('\n')
    ...
    for infile in args.infiles:
        do_phot(infile, photdata, plot=args.plot)
