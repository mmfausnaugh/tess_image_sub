...
def get_inputs(args):
    ...
    parser.add_argument("--image", help="Image for photometry")
    parser.add_argument("--photfile", help="COA results file")
    parser.add_argument("--outdir", default='./', help="location for output file")
    parser.add_argument("--stamp", action='store_true',help="if set, save a 30x30 pixel stamp around source")
...
def main():
    ...
    fout = open(os.path.join(args.outdir,'coa_phot.txt'),'a')
    fout.write('#' + '{:>14s} {:>10s} {:>15s} {:>15s} {:>15s} {:>15s}{:>15s} {:>15s} {:>15s} {:>15s} {:>15s} {:>15s}\n'.format(
        'catID','target','ra','dec','mag','final_cts','cts','bkg','median_bkg','Npix','crowd','fluxfrac'))
    ...
    for ii in range(len(target)):
        ...
        oa_rows_use = sp.array([  int(s) -1  for s in oa_rows[ii].split(',')[0:-1] ])
        oa_cols_use = sp.array([  int(s) -1  for s in oa_cols[ii].split(',')[0:-1] ])
        ...
        cts, bkg_median, bkg = photometry(image, 
                                          ...
                                          ref_row[ii],
                                          ref_col[ii],
                                          save_stamp=args.stamp)

        corrected_cts = (cts*crowding[ii])/flux_frac[ii]
        fout.write(fmt.format(catID[ii],
                              target[ii], 
                              ra[ii], dec[ii], mag[ii],
                              corrected_cts, cts, 
                              bkg, bkg_median, len(oa_rows_use),
                              crowding[ii],flux_frac[ii]
                          ))
